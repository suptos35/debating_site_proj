    ...
    <main class="max-w-4xl mx-auto mt-6 lg:mt-10 p-6">
        <div class="bg-white rounded-lg shadow-md p-8">
            <div class="flex items-center justify-between mb-6">
                ...
                <a href="@if($post->parent_id)/post/{{ $post->parent_id }}@else/@endif" class="text-blue-600 hover:text-blue-800 text-sm font-medium">
                    ← Back to @if($post->parent_id)Discussion@else Home @endif
                </a>
            </div>
            ...
            <form action="{{ route('posts.update', $post) }}" method="POST">
                ...
                <div class="mb-6">
                    ...
                    <textarea id="content" name="content" rows="8"
                              class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5"
                              placeholder="@if($post->parent_id)Your argument...@else Detailed content of your discussion... @endif" required>{{ old('content', $post->content) }}</textarea>
                    @error('content')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                @if(!$post->parent_id)
                    <!-- Categories for main posts only -->
                    <div class="mb-6">
                ...
                <div class="flex items-center justify-between pt-4 border-t border-gray-200">
                    <div class="flex space-x-3">
                        ...
                        <a href="@if($post->parent_id)/post/{{ $post->parent_id }}@else/@endif"
            ...
            <div class="mt-4 pt-4 border-t border-gray-200">
                <form action="{{ route('posts.destroy', $post) }}" method="POST" class="inline"
                      onsubmit="return confirm('Are you sure you want to delete this @if($post->parent_id)argument@else discussion @endif? @if(!$post->parent_id)This action cannot be undone and will also delete all related arguments and references. @endif');">
                    @csrf
                    @method('DELETE')
                    <button type="submit"
                        ...
                        Delete @if($post->parent_id)Argument @else Discussion @endif
                    </button>
                </form>
            </div>
        </div>
